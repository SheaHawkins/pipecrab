...
fn flush_selective_drops_unmarked_keeps_input_audio_in_order() {
    ...
    data_tx.try_send(DataFrame::Transcript(Transcript::user_final("A"))).unwrap();
...
fn flush_all_unmarked_returns_empty() {
    let (_, mut data_tx, mut inb) = lanes();
    data_tx.try_send(DataFrame::Transcript(Transcript::user_final("x"))).unwrap();
    data_tx.try_send(DataFrame::Transcript(Transcript::user_final("y"))).unwrap();
    assert!(inb.flush_data().is_empty());
}
...
fn flush_does_not_touch_sys_lane() {
    ...
    data_tx.try_send(DataFrame::Transcript(Transcript::user_final("drop me"))).unwrap();
