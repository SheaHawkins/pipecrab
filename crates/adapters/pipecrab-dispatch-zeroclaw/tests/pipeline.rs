//! One genuine round trip through the framework: a `dispatch_task` tool call
//! fed into a `PipelineBuilder`-built pipeline of the real
//! [`DispatchIngress`]/[`DispatchEgress`] stages — obtained from
//! `Dispatch::new(source, sink).into_stages()` — reaches this transport's
//! sink, posts a webhook turn, and the worker's completion comes back out past
//! the tail.
...
async fn a_tool_call_drives_the_transport_end_to_end() {
    ...
    Mock::given(method("POST"))
        .and(path("/webhook"))
        .respond_with(ResponseTemplate::new(200).set_body_json(reply("pipeline done")))
        .mount(&server)
        .await;
    ...
    // Drain past the tail until the turn's completion emerges, noting the task
    // id the acceptance assigned along the way.
