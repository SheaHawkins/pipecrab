//! [`HermesSink`]: the send half. Translates a [`DispatchCommand`] into a run
//! POST and emits the resulting [`Accepted`](DispatchEvent::Accepted) /
//! [`Rejected`](DispatchEvent::Rejected) onto the shared event channel.
...
/// The send half of the Hermes transport, a shared `Arc<Inner>` handle. Cloneable
/// so it can be held by an egress stage and still be inspected elsewhere.
#[derive(Clone)]
...
impl HermesSink {
    ...
    /// A cloned, oldest-first snapshot of every event emitted for `task_id`, or
    /// `None` if no such task exists.
    ///
    /// This is a sink-side convenience for inspection and debugging, bounded to
    /// the last few dozen events. It is *not* the system of record: the
    ...
    /// create-time `Rejected` precedes the task's existence and so appears in no
    /// ring, and a `Rejected` carries no `task_id` to look one up by.
...
#[async_trait]
impl DispatchSink for HermesSink {
