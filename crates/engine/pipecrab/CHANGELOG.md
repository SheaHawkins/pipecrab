# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [Unreleased]

## [0.5.1](https://github.com/SheaHawkins/pipecrab/compare/pipecrab-v0.5.0...pipecrab-v0.5.1) - 2026-07-23

### Added

- dispatch task interface ([#65](https://github.com/SheaHawkins/pipecrab/pull/65))

## [0.3.0](https://github.com/SheaHawkins/pipecrab/compare/pipecrab-v0.2.0...pipecrab-v0.3.0) - 2026-07-13

### Other

- group crates into engine/adapters/support with a layering gate ([#37](https://github.com/SheaHawkins/pipecrab/pull/37))

## [0.1.0](https://github.com/SheaHawkins/pipecrab/releases/tag/pipecrab-v0.1.0) - 2026-07-08

### Other

- scaffold model + engine crates, wire release-plz ([#20](https://github.com/SheaHawkins/pipecrab/pull/20))
- Add a cpal source+sink and demo app ([#13](https://github.com/SheaHawkins/pipecrab/pull/13))
- Wasm Compilation ([#9](https://github.com/SheaHawkins/pipecrab/pull/9))
- Inbound trait, hoist test utils, better containerization
- Add pipecrab core, alloc tests, ci
