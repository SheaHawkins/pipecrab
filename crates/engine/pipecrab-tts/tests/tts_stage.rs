...
fn barge_in_stops_emission_within_one_chunk() {
    block_on(async {
        ...
        let (ends, driver) = PipelineBuilder::new()
            .stage(pipecrab_tts::TtsStage::new(synth))
            .build()
            .start();
        ...
        let feed = async move {
            ...
            let _ = input
                .send_data(Transcript::agent_final("hello there").into())
                .await;
            ...
            emitted_rx
                .next()
                .await
                .expect("the synthesizer must emit one chunk");
            let _ = input
        ...
        assert!(
            ...
            "barge-in must reach the engine's cancel()"
        );
        assert!(
...
fn final_agent_transcript_becomes_audio_stream() {
    block_on(async {
        let (ends, driver) = PipelineBuilder::new()
            .stage(pipecrab_tts::TtsStage::new(FixedSynth { n: 3 }))
            .build()
            .start();
        let input = ends.input;
        let mut output = ends.output;
        ...
        let feed = async move {
            ...
            let _ = input
                .send_data(Transcript::agent_partial("typ").into())
                .await;
            // The agent's final speech: synthesized into audio.
            let _ = input
                .send_data(Transcript::agent_final("done").into())
                .await;
        };
        ...
        assert_eq!(
            audio, 3,
            "the final agent transcript yields one Audio frame per chunk"
        );
        assert_eq!(
            forwarded, 2,
            "user-final and agent-partial forward untouched"
        );
    });
}
