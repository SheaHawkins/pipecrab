...
- **Name availability.** The engine crates use generic names (`moonshine-web`,
  `silero-ort`, `kokoro-web`, `piper-web`, …). Confirm each is unclaimed on
  crates.io before the first release — a taken name fails `cargo publish`. If one
  is taken, rename the crate (e.g. prefix with `pipecrab-`) in its `Cargo.toml`
  and the workspace `members` list.
