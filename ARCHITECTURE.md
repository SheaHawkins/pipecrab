...
- `pipecrab-audio-web`, `pipecrab-vad-web`, `pipecrab-stt-web`, `pipecrab-lm-web`, `pipecrab-tts-web` — the browser adapters, mirror images of the native ones: wasm-only, so on any other target they compile to nothing rather than being excluded from the workspace. `WebAudioSource` captures through an `AudioWorklet` and `WebAudioSink` schedules playback on the audio clock; `SileroScorer` is a `SpeechScorer` over onnxruntime-web; `TransformersStt` is a `Transcriber` and `TransformersLm` a `LanguageModel` over Transformers.js; `KokoroTts` is a `Synthesizer` over kokoro-js. Each ships its browser-side glue as a `#[wasm_bindgen(module = "/js/…")]` snippet and takes the *loaded engine namespace* as a constructor argument, so no crate pins a CDN, a bundler layout, or an execution provider — where the engine comes from stays the application's call. See `examples/web-vad-stt` and `examples/web-e2e`.
...
The pipeline is one `!Send` thread, and `perform` must keep yielding so an interrupt can preempt it. Heavy work never runs inline: `offload(f).await` crosses to a `std::thread`. It is the only API with a `Send` bound — that bound is the thread-crossing boundary, not the pipeline.

On `wasm32` there is no thread to cross to, so `offload` runs `f` inline bracketed by a yield either side — enough for a queued system frame to be seen before and after, not a substitute for a thread. Work that needs one belongs in a JS Worker behind its capability trait instead: an engine's offload decision lives in its own impl, which is why the browser adapters can await a Worker while the stage above stays engine-neutral. `examples/web-e2e` moves Transformers.js inference into onnxruntime-web's proxy worker with one line of page config, and no adapter or stage changes.
...
That portability is exercised, not just checked: `examples/web-vad-stt` builds the same resample → VAD gate → STT pipeline as the desktop `stt-sherpa` example and runs it in a browser tab, with only the three backends swapped. The stages, the two lanes, the run loop, and the resampler are the same code on both. `examples/web-e2e` does the same for the full `e2e-voice-agent` loop — LM, sentence chunking, TTS, and barge-in included.

## Audio bridge

cpal's device callbacks run on a real-time thread that must never block, allocate, or lock; the pipeline is async. `pipecrab-audio-cpal::bridge` reconciles them with one lock-free `rtrb` ring per direction (moves `f32` samples) plus a `Signal` — an `AtomicWaker` and a "stream died" flag (moves wakeups). Capture: the RT callback produces, `CaptureRing` consumes; a full ring drops samples as an *overrun*. Playback: `PlaybackRing` produces with backpressure, the RT callback consumes; an empty ring outputs silence. `Signal::fail` wakes a parked task to `Err` on device loss. The bridge touches no cpal — only `rtrb` + `futures` — so it is backend-agnostic and tested without hardware. A `cpal::Stream` is `!Send`, but the interface is `Send`; so the stream is built and parked on a dedicated owning thread and `CpalSource`/`CpalSink` hold only the `Send` ring end (a server spawns one pump per session). This cpal backend is native-only; the browser audio path is `pipecrab-audio-web`.

The browser's counterpart to the real-time callback is an `AudioWorkletProcessor`, and `pipecrab-audio-web` reconciles it the same way: the worklet downmixes to mono, accumulates a fixed-size chunk, and transfers it to the main thread, where a bounded queue drops on overflow and counts the overrun. There is no stream-owning thread there — wasm32 is single-threaded and `MaybeSend` is vacuous, so the JS handles sit on the source itself. Playback needs no worklet: `WebAudioSink` schedules each chunk as an `AudioBufferSourceNode` on the context's clock, so the rendering thread plays it however busy the main thread is, and `cancel` stops every scheduled node.

## STT interface

`Transcriber` (`f32` in, text out) is the swappable capability; `SttStage` adapts one to a `Stage`, replacing a `DataFrame::Audio` with a `DataFrame::Transcript`. Audio crosses the async engine boundary as `Arc<[f32]>`, allowing worker-backed engines to retain or enqueue a chunk without copying its samples. Engines live behind the trait — native `ort`, browser Transformers.js — each owning its own offload, so the pipeline never names a concrete model.
