...
- `pipecrab-audio-web`, `pipecrab-vad-web`, `pipecrab-stt-web` — the browser adapters, mirror images of the native ones: wasm-only, so on any other target they compile to nothing rather than being excluded from the workspace. `WebAudioSource` captures through an `AudioWorklet`; `SileroScorer` is a `SpeechScorer` over onnxruntime-web; `TransformersStt` is a `Transcriber` over Transformers.js. Each ships its browser-side glue as a `#[wasm_bindgen(module = "/js/…")]` snippet and takes the *loaded engine namespace* as a constructor argument, so no crate pins a CDN, a bundler layout, or an execution provider — where the engine comes from stays the application's call. See `examples/web-vad-stt`.
...
That portability is exercised, not just checked: `examples/web-vad-stt` builds the same resample → VAD gate → STT pipeline as the desktop `stt-sherpa` example and runs it in a browser tab, with only the three backends swapped. The stages, the two lanes, the run loop, and the resampler are the same code on both.
