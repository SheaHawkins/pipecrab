...
- `pipecrab-dispatch-zeroclaw` — adapter crate: a `DispatchSource`/`DispatchSink` over a ZeroClaw gateway's synchronous `/webhook` endpoint. No poller — the gateway answers only when the agent turn finishes, so each command spawns a detached worker for one request. The minted `task_id` rides as `X-Session-Id`, and ZeroClaw's session memory chains follow-ups server-side. Native-only, like the Hermes adapter.
