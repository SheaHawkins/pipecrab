//! Transcribe speech from the default microphone with Sherpa VAD and
//! Moonshine v2.

#[cfg(any(target_os = "macos", target_os = "windows", target_os = "linux"))]
fn main() {
    if let Err(error) = desktop::run() {
        eprintln!("stt-sherpa-moonshine: {error}");
        std::process::exit(1);
    }
}
...
fn main() {
    eprintln!("stt-sherpa-moonshine requires a desktop OS (macOS, Windows, or Linux)");
    std::process::exit(1);
}
...
mod desktop {
    ...
    pub fn run() -> Result<(), Box<dyn Error>> {
        ...
        println!(
            "stt-sherpa-moonshine: input = {} @ {} Hz mono",
            source.device_name(),
            source.format().sample_rate
        );
        println!("stt-sherpa-moonshine: processing @ 16000 Hz mono");
        println!("stt-sherpa-moonshine: STT compute threads = {stt_threads}");
        println!(
            "stt-sherpa-moonshine: VAD threshold = 0.35, minimum speech = 100 ms, \
             pre-roll = 1000 ms, trailing silence = 500 ms"
        );
        match seconds {
            Some(seconds) => println!("stt-sherpa-moonshine: listening for {seconds} seconds"),
            None => println!("stt-sherpa-moonshine: listening until Ctrl-C"),
        }
        ...
        let pump_in = async move {
            ...
            loop {
                match source.next_chunk().await {
                    ...
                    Ok(None) => break,
                    Err(error) => {
                        eprintln!("stt-sherpa-moonshine: capture stopped: {error}");
                        break;
                    }
                }
            }
            ...
            while let Some(received) = output.recv().await {
                match received {
                    ...
                    Received::Sys(_, SystemFrame::Error { message, fatal: _ }) => {
                        eprintln!("stt-sherpa-moonshine: pipeline error: {message}")
                    }
        ...
        println!("stt-sherpa-moonshine: stopped");
        Ok(())
    }
                ...
                other => {
                    return Err(format!(
                        "unknown argument {other:?} (expected --vad-model, --encoder, \
                         --merged-decoder, --tokens, --stt-threads, or --seconds)"
                    ));
                }
            }
        }
